...
async def client_credentials_token(grant_type=Form(...),client_id=Form(...),client_secret=Form(...)):
	'\n    OAuth 2.0 Client Credentials Grant.\n    Used for M2M (machine-to-machine) authentication.\n    ';B='Basic';C='WWW-Authenticate';D='client_credentials'
	if grant_type!=D:raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,detail='Unsupported grant type')
	A=crud.get_client_by_client_id(client_id)
	if not A:raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail='Invalid client_id',headers={C:B})
	if not security.verify_password(client_secret,A.hashed_client_secret):raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail='Invalid client_secret',headers={C:B})
	if not A.is_active:raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,detail='Inactive client')
	if D not in A.grant_types:raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,detail='Grant type not allowed for this client')
